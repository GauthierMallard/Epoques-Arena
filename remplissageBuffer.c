...
void remplissageBuffer(BITMAP* decor,BITMAP* grillage,BITMAP* page,BITMAP *coeur,BITMAP *epee,BITMAP *chaussure,BITMAP *perso0w,BITMAP *perso1w,BITMAP *perso2w,BITMAP *perso3w,BITMAP *persocourt,BITMAP *fondlave,BITMAP *joueurSuivant,BITMAP *caseAttaquePrint,BITMAP *caseAttaqueEte,BITMAP *caseAttaqueAut,BITMAP *caseAttaqueHiv,t_joueur* joueurs,int jqj,int nbjoueur)
{
    ...
    masked_blit(grillage,page,0,0,0,0,640,640);
    masked_blit(epee,page,0,0,710,470,SCREEN_W,SCREEN_H);
    textprintf_ex(page, font, 750, 575, makecol(0, 0, 0),-1,"%d",joueurs[jqj].pa); // affiche pa du joueur

    if(joueurs[jqj].classe==2)
    {
        masked_blit(caseAttaquePrint,page,0,0,200,640,SCREEN_W,SCREEN_H);
    }
    else if(joueurs[jqj].classe==0)
    {
        masked_blit(caseAttaqueEte,page,0,0,200,640,SCREEN_W,SCREEN_H);
    }
    else if(joueurs[jqj].classe==3)
    {
        masked_blit(caseAttaqueAut,page,0,0,200,640,SCREEN_W,SCREEN_H);
    }
    else if(joueurs[jqj].classe==1)
    {
        masked_blit(caseAttaqueHiv,page,0,0,200,640,SCREEN_W,SCREEN_H);
    }
